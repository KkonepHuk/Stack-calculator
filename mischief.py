...
class Mischief(Compf):
    ...
    def compile(self, string):
        ...
        for token in self.data:
            ...
            # Некоммутативные операции всегда берем в группу и кладем на стек
            if token not in "+*":
            ...
            if first == token:
                ...
                # Те же махинации для случая, когда группой является только второй аргумент
                ...
                # Если операторы различаются, то берем все в группу и кладем на стек
                self.g.push(Group(token, [first, second]))
